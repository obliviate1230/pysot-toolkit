...
class UAVDTDataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        ...
        self.anno_files = sorted(glob.glob(
            os.path.join(dataset_root, 'anno/*_gt.txt')))
        # self.seq_dirs = [os.path.dirname(f) for f in self.anno_files]
        # self.seq_names = [os.path.basename(d) for d in self.seq_dirs]
        self.seq_names = os.listdir(dataset_root+'/data_seq')
        self.seq_names.sort()
        self.seq_dirs = [os.path.join(dataset_root+'/data_seq', n) for n in self.seq_names]
...
if __name__=='__main__':
    sys.path.append(os.getcwd())
    root_dir = os.path.abspath('/home/cps/zwq/siamesetracker/siamban/testing_dataset/UAVDT')
    dataset = UAVDTDataset(name='UAVDT', dataset_root=root_dir)